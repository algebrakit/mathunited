...
<xsl:template match="applet[@type='ggb']" mode="content">
    ...
    <iframe style="overflow:hidden" _type='ggb'>
        ...
        <xsl:attribute name="src">
            <xsl:value-of select="concat('../geogebra/',replace(@filename,'\.ggb','.html'))"/>
        </xsl:attribute>
    </iframe>
</xsl:template>
        ...
        <xsl:when test="$_sheetref_as_links_">
            <a target="_blank">
                <xsl:attribute name="href">
                    <xsl:value-of select="concat($subcomp,'-worksheet-', @item,'.html')"/>
                    <!-- <xsl:choose>
                        <xsl:when test="string-length($num) > 0">
                            <xsl:value-of select="concat($subcomp,'-worksheet-',$num,'-', @item)"/>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:value-of select="concat($subcomp,'-worksheet-', @item)"/>
                        </xsl:otherwise>
                    </xsl:choose> -->
                </xsl:attribute>
                <xsl:value-of select="."/>

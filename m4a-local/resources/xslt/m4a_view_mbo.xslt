...
<xsl:param name="ws_id"/>   <!-- id of worksheet, if applicable -->
...
<xsl:variable name="intraLinkPrefix">
    <xsl:choose>
        <xsl:when test="string-length($sector) > 0">
            <xsl:value-of select="concat('javascript:M4A_link(&quot;../../../',$sector,'/',$comp,'/',$subcomp,'/',$subcomp,'-')"/>
        </xsl:when>
        <xsl:otherwise>
...
<div class="headingDiv">
    <div class="headingContentDiv">
        <a>
            ...
            <img class="logo" align="middle"  height="33" border="0">
                <xsl:attribute name="src"><xsl:value-of select="concat($css_js_base, 'sources_ma/LogoM4Ainvlak.gif')"/></xsl:attribute>
            </img>
        </a>
...
<xsl:template match="explore[@type=$sector or (not($sector) and string-length(@type)=0)]">
    ...
    <xsl:for-each select="include">
        <xsl:apply-templates select="document(concat($docbase,@filename))/exercise">
            <xsl:with-param name="options" select="$options"/>
            <xsl:with-param name="num" select="concat('V', position())"/>
    ...
    <a>
        ...
        <xsl:choose>
            <xsl:when test="$sector=@id">
                ...
                <xsl:attribute name="href"><xsl:value-of select="concat('javascript:M4A_link(&quot;../../../no-sector/',$comp,'/',$subcomp,'/index.html&quot;)')"/></xsl:attribute>
            </xsl:when>
            <xsl:otherwise>
                <xsl:attribute name="href"><xsl:value-of select="concat('javascript:M4A_link(&quot;../../../', @id, '/',$comp,'/',$subcomp,'/index.html&quot;)')"/></xsl:attribute>
...
<xsl:template match="application[@type=$sector or (not($sector) and string-length(@type)=0)]">
    ...
    <xsl:for-each select="exercises/include">
        <xsl:apply-templates select="document(concat($docbase,@filename))/exercise">
            <xsl:with-param name="options" select="$options"/>
            <xsl:with-param name="num" select="concat('A', position())"/>
...
<xsl:template match="test">
    ...
    <xsl:for-each select="exercises/include">
        <xsl:apply-templates select="document(concat($docbase,@filename))/exercise">
            <xsl:with-param name="options" select="$options"/>
            <xsl:with-param name="num" select="concat('T', position())"/>
            <xsl:with-param name="is-open">true</xsl:with-param>
        </xsl:apply-templates>
    </xsl:for-each>
</xsl:template>
...
<xsl:template match="exercise">
    ...
    <div>
        ...
        <div class="exercise-heading">
            Opgave <xsl:value-of select="$num"/> <span class="opgave-title-span"><xsl:value-of select="title"/></span> <div class="opgave-label-button"/>
        </div>

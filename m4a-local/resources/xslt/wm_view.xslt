...
<xsl:variable name="overviewRef">https://wageningse-methode.nl/methode/het-lesmateriaal/</xsl:variable>
<xsl:variable name="intraLinkPrefix">
    <xsl:value-of select="concat('javascript:M4A_link(&quot;',$subcomp,'-')"/>
</xsl:variable>

<xsl:variable name="indexDoc" select="document(concat($docbase,'../index.xml'))"/>
...
<head>    
    ...
    <link type="text/css" rel="Stylesheet">
        <xsl:attribute name="href"><xsl:value-of select="concat($css_js_base,'js/lib/jquery-ui-1.12.1.custom/jquery-ui.min.css')"/></xsl:attribute>
    </link>
    <script type="text/javascript">
        <xsl:attribute name="src"><xsl:value-of select="concat($css_js_base,'js/lib/jquery-ui-1.12.1.custom/jquery-ui.min.js')"/></xsl:attribute>
    </script>
    <script type="text/javascript" src="javascript/lib/touchpunch.js">
        <xsl:attribute name="src"><xsl:value-of select="concat($css_js_base,'js/lib/touchpunch.js')"/></xsl:attribute>
    </script>
    <link rel="stylesheet" type="text/css">
        <xsl:attribute name="href"><xsl:value-of select="concat($css_js_base,'css/grid.css')"/></xsl:attribute>
    </link>
    ...
    <link rel="stylesheet" type="text/css">
        <xsl:attribute name="href"><xsl:value-of select="concat($css_js_base,'css/basis_wm.css')"/></xsl:attribute>
    </link>
    <link rel="stylesheet" type="text/css">
    ...
    <script type="text/javascript">
        <xsl:attribute name="src"><xsl:value-of select="concat($css_js_base,'js/MathUnited_wm.js')"/></xsl:attribute>
    </script>
                                ...
                                <xsl:if test="$subcomponent_preceding_id != ''">
                                    <a>
                                        <xsl:attribute name="href">
                                            <xsl:value-of select="concat('../',$subcomponent_preceding_id,'/',$subcomponent_preceding_id,'.html')"/>
                                        </xsl:attribute>
                            &lt;
                                    </a>
                                </xsl:if>
                        ...
                        <div class="headingContentDiv grid_6">
                            ...
                            <xsl:if test="$subcomponent_following_id != ''">
                                <span id="next-subcomponent">
                                    <a>
                                        <xsl:attribute name="href">
                                            <xsl:value-of select="concat('../',$subcomponent_following_id,'/',$subcomponent_following_id,'.html')"/>
                                        </xsl:attribute>    
                                        &gt;
                                    </a>
                                </span>
                        ...
                        <div class="menu-item-div">
                            <xsl:choose>
                                <xsl:when test="$item='answers'">
                                     <a>
	                			       	<xsl:attribute name="href">
                                            <xsl:value-of select="concat('../',$subcomp,'/',$subcomp,'.html')"/>
        		                        </xsl:attribute>
                                ...
                                <xsl:otherwise>
                                    <a>
                                        <xsl:attribute name="href">
                                            <xsl:value-of select="concat('../',$subcomp,'/',$subcomp,'-answers.html')"/>
                                        </xsl:attribute>
            ...
            <xsl:choose>
                        ...
                        <div class="exercise-heading-wrapper {exercise[position()=$currentPos]/@level}">
                            ...
                            <div class="icon-container">
                                ...
                                <xsl:if test="exercise[position()=$currentPos]/worksheets/worksheet">
                                    <a target="_blank">
                                        <xsl:attribute name="href">
                                            <xsl:value-of select="concat('../',$subcomp,'/',$subcomp,'-worksheet-', exercise[position()=$currentPos]/@num,'.html')"/>
                ...
                <xsl:otherwise>
                    ...
                    <div class="icon-container active">
                        ...
                        <a target="_blank">
                            <xsl:attribute name="href">
                                <xsl:value-of select="concat('../',$subcomp,'/',$subcomp,'-worksheet-', @num,'.html')"/>
